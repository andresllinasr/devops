...
Snippet for automating new service creation in Nginx (Experimental)
...
if [ -z "$NAME" ] || [ -z "$PORT" ]; then
  echo "Usage: $0 <subdomain> <internal-port>"
  exit 1
fi

# Create Nginx block
NGINX_PATH="/etc/nginx/sites-enabled/$NAME.conf"
CERT_PATH="/etc/nginx/certs/wildcard.andresllinasr.com"

cat <<EOF | sudo tee $NGINX_PATH
server {
    listen 443 ssl;
    server_name $NAME.andresllinasr.com;

    ssl_certificate $CERT_PATH/wildcard.andresllinasr.com.cer;
    ssl_certificate_key $CERT_PATH/wildcard.andresllinasr.com.key;
...
echo "✅ Service available at https://$NAME.andresllinasr.com"
